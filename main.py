...
def preprocessing_data(csv_path: str) -> pd.DataFrame:
    ...
    df["Embarked"] = df.Embarked.map(
        {"C": 0, "Q": 1, "S": 2}
    )  # Encoding the embark port
    df = df.drop(
        ["Name", "Cabin", "Ticket"], axis=1
    )  # Dropping the names, cabins and Ticket
...
for epoch in range(1200):
    ...
    print(
        f"Epoch {epoch + 1} — avg loss: {avg_loss:.4f} - Accuracy: {epoch_accuracy:.4f}"
    )
...
result_df = pd.DataFrame({"PassengerId": passenger_ids, "Survived": class_labels})
df = result_df
